...
class Category3View: UIView {
    ...
    func stackhelper(with items: [CutleryModel]) {
        for item in items {
            guard let id = item.id, let title = item.title else { return }
            optionsStackView.addArrangedSubview(CategoryRow(id: id, title: title, hasPromo: false, price: item.price ?? 0, promoPrice: nil, category: .onlyOne, delegate: nil))
        }
    }
